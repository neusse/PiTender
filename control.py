...
# prototype_mode = False # use this to actualy use the relay and pumps
...
def Pour(pump_number, waitTime, platform):
    ...
    WriteLog(f' * Thread: {pump_number} Started for {waitTime} seconds.')
    ...
    for x in range(waitTime):
        ...
        if(stop_threads):
            ...
            WriteLog(f' * Thread: {pump_number} cancelling.')
            break
    platform.DeActivatePump(pump_number)
    print(f' * Thread: {pump_number} finished.')
    WriteLog(f' * Thread: {pump_number} finished.')


def PercentDone(total_runtime):
...
def PourDrink(drink_name):
    ...
    WriteLog(f"Pouring Drink {drink_name}")
    ...
    if (drink_name in drink_db['drinks']):
        ...
        WriteLog(f'Starting to prepare {drink_name}')
        ...
        for drink_ingredient, pump_runtime in drink_db['drinks'][drink_name]['ingredients'].items():
            ...
            total_runtime = max(total_runtime, int(
                pump_runtime * (settings['flowrate'] / 100)))

        print(f'Total Runtime: {total_runtime}')
        WriteLog(f'Total Runtime: {total_runtime}')
        ...
        for drink_ingredient, pump_runtime in drink_db['drinks'][drink_name]['ingredients'].items():
            ...
            # Init thread
            calculated_pump_runtime = max(
                1, int((pump_runtime * (settings['flowrate'] / 100))))
            print(
                f'Calculated Pump Runtime for {drink_ingredient} = {calculated_pump_runtime}')
            pump_t = threading.Thread(target=Pour, args=(
        ...
        # WriteLog('Finished. Cleaning up status file.')
        ...
        WriteLog('Error, no drinks match that name.')
            ...
            if (pin_number != 0) and (status['control']['stop'] == 0):
                ...
                for index in range(20):
                    ...
                    if (status['control']['stop'] == 0):
                        progress += 1
                        status['status']['progress'] = int(
                            100 * (progress / total_runtime))
                        WriteStatus(status)
        ...
        while True:
            ...
            if status['status']['active'] == 0:
                if status['control']['start'] == 1:
                    event = 'Drink requested: ' + \
                        status['control']['drink_name']
                    WriteLog(event)
                    print("Pouring drink")
                    PourDrink(status['control']['drink_name'])
                elif status['control']['clean'] != "":
                    event = 'Clean requested for pump: ' + \
                        str(status['control']['clean'])
                    WriteLog(event)
